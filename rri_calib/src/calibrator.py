...
class RRIPaths:
    ...
    obs_dir: Path = Path("rri_calib/obs")
    sim_dir: Path = Path("calcHydro.txt")
    plot_dir: Path = Path("rri_calib/out/plot")
    ...
    def plot_hydro(
        ...
        if objFun == "RMSE":
            for fname, sim, obs in zip(self.station_files, sim_list, obs_list):
                ...
                plt.plot(t, obsv, color="black", lw=1.5, label="Obs_Disc")
                plt.plot(t, simv, color="tab:red", lw=1.5, label="Sim_Disc", alpha=0.8)
                plt.xlabel("Output step")
                plt.ylabel("Discharge [m³/s]", fontweight='bold')
                plt.title(f"Hydrograph at {sta[5:].capitalize()} Station\n", fontsize=12, fontweight='bold')
                plt.text(0.5, 1.080, f"R²: {r2:.2f}, RMSE: {objfunc:.2f}", color='gray',
                 fontsize=9, ha='center', va='top', transform=plt.gca().transAxes)
                
                # plt.legend(ncol=2, fontsize=10)
                plt.legend(ncol=2, fontsize=10, loc="upper center", bbox_to_anchor=(0.5, -0.25), frameon=True)
            ...
            for fname, sim, obs in zip(self.station_files, sim_list, obs_list):
                ...
                plt.ylabel("Discharge [m³/s]", fontweight='bold')
                plt.title(f"Hydrograph at {sta[5:].capitalize()} Station\n", fontsize=12, fontweight='bold')
                plt.text(0.5, 1.080, f"R²: {r2:.2f}, MAE: {objfunc:.2f}", color='gray',
                 fontsize=9, ha='center', va='top', transform=plt.gca().transAxes)
                # plt.legend(ncol=2, fontsize=10)
                plt.legend(ncol=2, fontsize=10, loc="upper center", bbox_to_anchor=(0.5, -0.25), frameon=True)
                plt.grid(axis="y", ls="--")
